$(document).ready(function () {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/admin_report_items/fetch_admin_report_items',
            data: {
...
crud.ajax = {
    del_data: function (id, cb) {
        var url = '/admin_report_items/del_admin_report_items',
            params = {
                id: id
            }
        ...
        var url = '/admin_report_items/save_admin_report_items',
            params = {
                items: items
            }
        ...
        var url = '/admin_report_items/get_admin_report_items',
            params = {
                id: id
            }
...
crud.set_after_update = function (items, row_id) {
    ...
    row_id.find("td:eq(3)").html(items.group);
    row_id.find("td:eq(4)").html(items.id);

}
crud.set_after_insert = function (items, id) {

    $('<tr name="row' + (id + 1) + '"><td>' + id + '</td>' +
        '<td>' + items.id + '</td>' + '<td>' + items.name + '</td>' + '<td>' + items.link + '</td>' + '<td>' + items.group + '</td>' + '<td>' + items.id + '</td>' +
...
function validate(items) {
     ...
     if (!items.name) {
        swal("กรุณาระบุชื่อรายงาน");
        $("#name").focus();
    } else if (!items.link) {
        swal("กรุณาระบุLink");
        $("#link").focus();
    } else if (!items.group) {
        swal("กรุณาระบุกลุ่มรายงาน");
        $("#group").focus();
    }
    else {
        return true;
