$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/admin_report_group/fetch_admin_report_group',
            data: {
...
crud .ajax = {
    del_data:function (id,cb){
        var url = '/admin_report_group/del_admin_report_group',
            params = {
                id: id
            }
        ...
        app.ajax(url, params, function (err, data) {
            err ? cb(err) : cb(null, data);
             ...
             var url = '/admin_report_group/save_admin_report_group',
                 params = {
                     items: items
                 }
             ...
             app.ajax(url, params, function (err, data) {
                 err ? cb(err) : cb(null, data);
                   ...
                   var url = '/admin_report_group/get_admin_report_group',
                       params = {
                           id: id
                       }
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.name) { swal("กรุณาระบุ");$("#name").focus();}
    else{
        return true;
    }
