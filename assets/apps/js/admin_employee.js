...
crud.set_after_update = function (items, row_id) {
    ...
    row_id.find("td:eq(7)").html(items.hospcode);
    row_id.find("td:eq(8)").html(items.tel);
    row_id.find("td:eq(9)").html(items.line);
    row_id.find("td:eq(10)").html(items.facebook);
    row_id.find("td:eq(11)").html(items.active);

}
crud.set_after_insert = function (items, id) {

    $('<tr name="row' + (id + 1) + '"><td>' + id + '</td>' +
        '<td>' + items.id + '</td>' + '<td>' + items.prename + '</td>' + '<td>' + items.name + '</td>' + '<td>' + items.sex + '</td>' + '<td>' + items.cid + '</td>' + '<td>' + items.position + '</td>' + '<td>' + items.employee_type + '</td>' + '<td>' + items.hospcode + '</td>' + '<td>' + items.tel + '</td>' + '<td>' + items.line + '</td>' + '<td>' + items.facebook + '</td>' + '<td>' + items.active + '</td>' +
...
crud.set_update = function (data, row_id) {
    ...
    $("#line").val(data.rows["line"]);
    $("#facebook").val(data.rows["facebook"]);
...
function validate(items) {
 ...
 if (!items.prename) {
        ...
        swal("กรุณาระบุLineID");
        $("#line").focus();
    } else if (!items.facebook) {
        swal("กรุณาระบุFacebook");
        $("#facebook").focus();
