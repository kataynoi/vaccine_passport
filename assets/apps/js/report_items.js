$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/report_items/fetch_report_items',
            data: {
...
crud .ajax = {
    del_data:function (id,cb){
        var url = '/report_items/del_report_items',
            params = {
                id: id
            }
        ...
        app.ajax(url, params, function (err, data) {
            err ? cb(err) : cb(null, data);
             ...
             var url = '/report_items/save_report_items',
                 params = {
                     items: items
                 }
             ...
             app.ajax(url, params, function (err, data) {
                 err ? cb(err) : cb(null, data);
                   ...
                   var url = '/report_items/get_report_items',
                       params = {
                           id: id
                       }
...
crud.set_after_update = function (items,row_id) {
    ...
    row_id.find("td:eq(0)").html(items.id);row_id.find("td:eq(1)").html(items.name);row_id.find("td:eq(2)").html(items.link);row_id.find("td:eq(3)").html(items.group);row_id.find("td:eq(4)").html(items.active);

}
crud.set_after_insert = function (items,id) {

            $('<tr name="row'+(id+1)+'"><td>'+id+'</td>' +
                '<td>' +items.id+'</td>' +'<td>' +items.name+'</td>' +'<td>' +items.link+'</td>' +'<td>' +items.group+'</td>' +'<td>' +items.active+'</td>' +
...
crud.set_update = function (data,row_id) {
    $("#row_id").val(row_id);
    $("#id").val(data.rows["id"]);$("#name").val(data.rows["name"]);$("#link").val(data.rows["link"]);$("#group").val(data.rows["group"]);$("#active").val(data.rows["active"]);
}
...
$('#btn_save').on('click', function (e) {
    ...
    items.id=$("#id").val();items.name=$("#name").val();items.link=$("#link").val();items.group=$("#group").val();items.active=$("#active").val();
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.name) { swal("กรุณาระบุชื่อรายงาน");$("#name").focus();}else if (!items.link) { swal("กรุณาระบุLink");$("#link").focus();}else if (!items.group) { swal("กรุณาระบุกลุ่มรายงาน");$("#group").focus();}else if (!items.active) { swal("กรุณาระบุการใช้งาน");$("#active").focus();}
    else{
        return true;
    }
