$(document).ready(function () {
  var dataTable = $("#table_data").DataTable({
    ...
    pageLength: 50,
    ajax: {
      url: site_url + "/whitelist_student/fetch_whitelist_student",
      data: {
...
crud.ajax = {
  del_data: function (id, cb) {
    var url = "/whitelist_student/del_whitelist_student",
      params = {
        id: id,
      };
  ...
  save: function (items, cb) {
    var url = "/whitelist_student/save_whitelist_student",
      params = {
        items: items,
      };
  ...
  save_org: function (items, cb) {
    var url = "/whitelist_student/save_org",
      params = {
        items: items,
      };
  ...
  get_update: function (id, cb) {
    var url = "/whitelist_student/get_whitelist_student",
      params = {
        id: id,
      };
  ...
  crud.ajax.get_person_by_cid(cid, function (err, data) {
    ...
    if (!err) {
      if (data.check_vaccine) {
        ...
        app.clear_form();
        return;
      } else if (data.rows["age_y"] < 12 || data.rows["age_y"] > 17) {
        swal("บุคคลนี้ไม่อยู่ในช่วงอายุ 12-17 ปี");
        app.clear_form();
        return;
      } else if (data.rows) {
        $("#cid").val(data.rows["CID"]);
